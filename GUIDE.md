...
2. [このプロジェクトでの学び方](#2-このプロジェクトでの学び方)
3. [バイブコーディングとの付き合い方](#3-バイブコーディングとの付き合い方)
4. [使う技術の紹介](#4-使う技術の紹介)
5. [設計の「なぜ」解説](#5-設計のなぜ解説)
6. [セキュリティ解説](#6-セキュリティ解説)
7. [実装ガイド](#7-実装ガイド)
8. [デプロイガイド（Vercel）](#8-デプロイガイドvercel)
9. [iPad運用ガイド](#9-ipad運用ガイド)
10. [よくあるハマりどころ](#10-よくあるハマりどころ)
...
「アプリ開発が初めて」でも大丈夫です。このプロジェクトは **「作ってから理解する」** を基本スタンスにしています。

### 進め方

1. **まず動かす**: よくわからないままでいい。AI に言われた通りに作って、とにかく自分のアプリを Vercel に上げる
2. **定例会で解読する**: 動いたコードを毎週少しずつ読み解く。「この行は何をしているのか」を1つ持ってくるだけでいい
3. **エラーを教材にする**: 壊れたとき・AI に直してもらったときが一番の学びの機会。「何が起きていたのか」を定例会で共有する

### 定例会に持ってくるネタの例

- AI が書いたけど意味が分からないコードの断片
- エラーが出て直してもらったが何が起きていたのか分からない
- GEMINI.md のルールで「なぜこれが禁止なのか」疑問に思ったもの

### 「理解してから作る」より「作ってから理解する」

座学で HTML → CSS → JavaScript → React と積み上げてから作ると、理解はできても「なぜこれを学んでいるのか」が見えにくくなります。このプロジェクトでは自分のアプリが部室の iPad に映った時点で動機が生まれ、そこから逆引きで学ぶ方が記憶に残ります。

> **注意**: 「よくわからんまま」が続きすぎるとルール（GEMINI.md）を守る理由が分からなくなります。定例会で早めに「なぜそのルールがあるか」を聞くようにしてください。
...
> **重要**: `pointer-events: none` により、`/widget` 内のボタン・リンク・タブは一切タップできません。バイブコーディングで「ボタンを作ったのにタップしても無反応」という原因不明のデバッグに陥りがちです。`/widget` は**表示専用**で、インタラクティブな要素は `/`（全画面ページ）にのみ置いてください。
