...
岡山大学DS部の部室にあるiPadで、24時間表示し続けるダッシュボード画面を作ります。
...
各アプリは**別々のリポジトリ・別々のチーム**が担当します。最後にダッシュボードが全アプリを統合します。
...
1. **Gitコンフリクトが減る**: 別リポジトリなので、他チームの変更と衝突しない
2. **各チームが独立して動ける**: 進捗・デプロイが互いに影響しない
3. **新入生の学習負荷が低い**: 大きなリポジトリ全体を理解する必要がない
4. **失敗が閉じ込められる**: 一つのアプリが壊れても他に波及しない

### 4-2. なぜiframe統合なのか
代替案としてJSON API方式（各アプリがJSONを返し、ダッシュボード側がUIを描く）も検討しましたが、以下の理由で却下：

- **新入生がAIで作ったUIがダッシュボードで捨てられてしまう**
- 教育的な達成感が削がれる（「自分が作った画面がダッシュボードに出てる！」を大事にしたい）
- iframeなら各チームの成果物をそのまま表示できる
...
3. 自分のGitHubリポジトリを選んで **Import**
4. Framework Preset は自動で **Next.js** が選ばれる
5. **Environment Variables** セクションで必要な環境変数を入力：
   - `NEXT_PUBLIC_DASHBOARD_URL` = `https://dsc-dashboard.vercel.app`
   - `WEATHER_API_KEY` = 取得したキー
6. **Deploy** をクリック → 数分待つと本番URLが発行される
...
### 8-4. Wi-Fi切断時の挙動
Wi-Fi切れ + 5分後の自動リロードが重なると画面が真っ白になります。対策として、ダッシュボードにオフライン表示を用意するのが理想（今後の改善課題）。
